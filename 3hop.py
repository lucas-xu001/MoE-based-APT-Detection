...
def train_gnn_transformer(
    ...
    for batch in train_data.seq_batches(batch_size=BATCH):
        ...
        for src1, dst1 in zip(src, pos_dst):
            ...
            neighbor1_mask = (target_nodes.unsqueeze(0) == neighbor1.unsqueeze(1)).any(dim=0)
            filtered_edges1 = edge_index[1, neighbor1_mask]
            neighbor2_mask = (target_nodes.unsqueeze(0) == neighbor2.unsqueeze(1)).any(dim=0)
            filtered_edges2 = edge_index[1, neighbor2_mask]

            tmp1 = torch.cat([assoc[src1].unsqueeze(0), neighbor1, filtered_edges1], dim=-1).unique()
            tmp2 = torch.cat([assoc[dst1].unsqueeze(0), neighbor2, filtered_edges2], dim=-1).unique()
